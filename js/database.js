// Banco de dados dos produtos

let cervejas= [
  {
    id: 1,
    img: "img/12393398-removebg-preview.png",
    nameItem: "Cerveja Lagunitas",
    description:
    "A Lagunitas IPA consiste em uma versão única de um estilo antigo. ",
    value: 53.00,
    addCart: "Adicionar ao carrinho",
    tag: ["Ipa"],
    botao:"btn1" 
  },
  {
    id: 2,
    img: "img/iron.png",
    nameItem: " BodeBrow Iron Mainden",
    description:
    "Cerveja oficial do Iron Maiden, cerveja artesanal tipo IPA.",
    value: 40.00,
    addCart: "Adicionar ao carrinho",
    tag: ["Ipa"],
    botao:"btn2" 
  },
  {
    id: 3,
    img: "img/lager.png.png",
    nameItem: "Cerveja Casa Queen's",
    description:
   " Cerveja equilibrada que possui corpo e sabor. ",
    value: 77.00,
    addCart: "Adicionar ao carrinho",
    tag: ["Lager"],
    botao:"btn3" },
  {
    id: 4,
    img: "img/Cerveja-Lager-Premium-Puro-Malte-Bierville-Lata-350ml-removebg-preview.png",
    nameItem: "Cerveja BierVille",
    description:
    "Cerveja fermentada e armazenada em baixas temperaturas. ",
    value: 45.00,
    addCart: "Adicionar ao carrinho",
    tag: ["Lager"],
    botao:"btn4" },
    
  {
    id: 5,
    img: "img/images-removebg-preview.png",
    nameItem: "Kit Pilsen",
    description:
    "O tipo pilsen é o mais conhecido e consumido pelos brasileiros. ",
    value:9.00,
    addCart: "Adicionar ao carrinho",
    tag: ["Pilsen"],
    botao:"btn5" 
  },{
    id: 6,
    img: "img/61-7F5lY2HS._AC_SL1001_-removebg-preview.png",
    nameItem: "Cerveja Eisenbahn",
    description:
    "De baixa fermentação e com teor alcoólico de 4,84%.",
    value: 16.00,
    addCart: "Adicionar ao carrinho",
    tag: ["Pilsen"],
    botao:"btn6" 
  },
]


